  ...
  >
    ...
    >
      ...
      >
        ...
        >
          Verifikasi
          <span class="font-bold text-primary-300">Email</span>
        </h1>
        <p class="text-base sm:text-lg text-center">
          Masukkan <span class="font-bold">alamat email</span> yang Anda gunakan
          untuk mendaftar. Kami akan mengirimkan email berisi
          <span class="font-bold">tautan dan langkah-langkah</span> untuk
...
const submitForm = async () => {
  ...
  const success = await authStore.forgetPasswordVerifyEmail(form);
  ...
  if (success) {
    router.push({
      ...
      query: { showOTPSentSuccessAlert: "true", email: form.email },
    });
  } else {
