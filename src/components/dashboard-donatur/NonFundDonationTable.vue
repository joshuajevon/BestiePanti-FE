<template>
  <div>
    ...
    class="error-message text-center mt-20 mb-20"
    >
      *Anda belum memiliki donasi Yuk donasi sekarang juga!*
      <a href="/panti" class="underline text-blue-500"
        >Lihat Panti Asuhan</a
      >.
    </p>
...
const filteredNonFundDonationList = computed(() => {
  return nonFundDonationList.value.filter((donation) => {
    ...
    return matchesPantiSearch && matchesStatus && matchesDonationType;
  });
});

watch([searchPanti, selectedStatuses, selectedDonationTypes], () => {
  currentPage.value = 1;
});
